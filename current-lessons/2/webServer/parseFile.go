...
func uploadPage(w http.ResponseWriter, r *http.Request) {
	...
	_, _ = fmt.Fprintf(w, "header.Filename %v\n", header.Filename)
	_, _ = fmt.Fprintf(w, "header.Header %#v\n", header.Header)

	hasher := md5.New()
	_, _ = io.Copy(hasher, file)

	_, _ = fmt.Fprintf(w, "md5 %x\n", hasher.Sum(nil))
}
...
/*
curl -v -X POST -H "Content-Type: application/json" -d '{"id": 2, "user": "name"}' http://localhost:8080/raw_body
*/
...
func uploadRawBody(w http.ResponseWriter, r *http.Request) {
	...
	_, _ = fmt.Fprintf(w, "content-type %#v\n",
