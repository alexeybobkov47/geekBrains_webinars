...
func main() {
	...
	if err := e.UpdateTask(9, "do hw", true); err != nil {
		log.Print(err)
		return
	}

	if err := e.DeleteTask(9); err != nil {
		log.Print(err)
		return
	}
}
...
func (e Explorer) CreateList(list TaskList) error {
	_, err := e.db.Exec("insert into task_list_app.lists (name, description) values (?,?)", list.Name, list.Description)
	return err
}
...
func (e Explorer) DeleteTask(id int) error {
	_, err := e.db.Exec("DELETE FROM `task_list_app`.`tasks`  WHERE (`id` = ?)", id)
